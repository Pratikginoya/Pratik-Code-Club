...
<script>
  $(document).ready(function(){
    ...
    $('.note-color-all button[data-toggle="dropdown"]').remove();
    $('.note-para .btn-group').remove();
    $('.note-table').remove();
    $('.note-insert').remove();
    $('.note-view button[aria-label="Help"]').remove();
    $('.textarea-text-view').find('div.note-editable').css('height','200px');
  });
</script>

<script>
  $(function () {
      $('#CodeDetailsTable').DataTable({
        "paging": true,
        "lengthChange": true,
        "searching": true,
        "ordering": true,
